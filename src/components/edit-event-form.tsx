...
export default function EditEventForm({ event, categories }: EditEventFormProps) {
  ...
  const handlePublishToggle = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setFormErrors([result.error || 'Error al cambiar estado de publicación'])
        return
      }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Button
            ...
            className={event.isPublished ? "" : "bg-gradient-to-r from-[#0053CC] to-[#01CBFE]"}
          >
            {publishLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
            {event.isPublished ? (
              <>
            ...
            <div className="flex items-center gap-2 text-yellow-800 dark:text-yellow-200">
              ...
              <div>
                ...
                <p className="text-sm">
                  ...
                  Ten cuidado al hacer cambios importantes como fecha, ubicación o capacidad.
                </p>
              </div>
            </div>
          </CardContent>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    min={hasTicketsSold ? undefined : new Date().toISOString().split('T')[0]}
                  />
                  {hasTicketsSold && (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="text-center">
                <h3 className="font-bold text-lg">{formData.title || 'Título del evento'}</h3>
                <p className="text-sm text-muted-foreground">{formData.location || 'Ubicación'}</p>
                <p className="text-lg font-bold text-[#0053CC] mt-2">
                  {formData.isFree ? 'Gratis' : `${formData.price.toLocaleString('es-CL')} CLP`}
                </p>
              </div>
              
              <div className="text-xs text-muted-foreground space-y-1">
                <p>📅 {formData.startDate || 'Sin fecha'} {formData.startTime || '00:00'}</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{event.capacity - event._count.tickets}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Ocupación:</span>
                <span className="font-medium">{Math.round((event._count.tickets / event.capacity) * 100)}%</span>
              </div>
            </CardContent>
          </Card>
